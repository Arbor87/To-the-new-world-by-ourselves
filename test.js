...
function checkCollision() {
    ...
    function shouldHandleGameTouch(e) {
        const target = e.target;
        const isControlTouch = target.closest && target.closest('.controls');
        const isModalTouch = target.closest && target.closest('.quiz-modal.active, #audioPrompt, #model-preview-container');
        return (startgame.classList.contains('active') || gameContainer.classList.contains('active') || isControlTouch) && !isModalTouch;
    }

    function handleTouchButton(e) {
        const target = e.target;
        if (!target.closest) return false;

        if (target.closest('#leftBtn')) {
            if (positionX > 10) {
                positionX -= 5;
                player.style.left = `${positionX}%`;
                checkCollision();
    ...
    document.addEventListener('touchstart', (e) => {
        if (!shouldHandleGameTouch(e)) return;
        // 只在游戏操作区阻止浏览器滚动，避免影响剧情选项点击
        e.preventDefault();
